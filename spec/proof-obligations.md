...
| R7 | ✅ 完成（adversarial_r7.rs 15 测试，迭代 2，E2E 累计 203）：RFC-05/06/07/08/09 修复面 + R-6 fork_snapshot + DX 层六面攻击；发现：dx mutex 不可重入（已修）、TcpShutdown infer_usage（已修，后裁决空集） | ✅ 完成（formal R7，迭代 2）：**P4 升级「有效（附范围声明）」**——RFC-05 句柄活性反例闭环 + 取消传播（undo 回滚/线性快照/锁释放）证据齐备；P1 承诺域 ≤64 与代码一致；A4 双回滚（取消×失败）无冲突；do_! 展开 ≡ 手写 CPS 链等价性成立 | dx mutex 空声明修复；SendSignal 可重复；TcpShutdown Own→空集（40a62a6）；P4 升级证据链 | **迭代 2 收官**：P1/P2/P3/P5 有效附声明维持、**P4 升级有效（附范围声明）**——A1-A7 × P1-P5 全部有效/部分中仅剩 P4 范围例外（R7-A/B 宽限耗尽）与 P3 物理层（make_mut 阶段 3） |
| R8 | ✅ 完成（adversarial_r7ab.rs 4 测试，迭代 3，E2E 累计 203）：R7-A/B 核销轮——管道读取消泄漏缺陷锁定 + 宽限耗尽孤儿占坑缺陷锁定 + 宽限内 join 正向 + 轮换基线 | ✅ 完成（formal R8，迭代 3）：P1-P5 终态复核——P4「有效（附范围声明）」维持，范围声明修订（R7-A 已核销、R7-B 收窄为耗尽路径残余）；P5 边界句载体更新（README 已知限制表）；A1 共享 reactor 语义保持成立（合并顺序/取消粘性/物理 IO 串行不变，fork_snapshot 契约回退一致） | R7-A/B 修复核销（8e842d6 TakeHandleGuard + ForkJoinMerge）；计数 418/50+3/186（实测）；prof_fork 分解；perf/it3 基线 | **迭代 3 收官**：R7-A/B 修复落地、性能二轮（共享 reactor）、P4 例外收窄为 R7-B 耗尽路径残余；P3 物理层 make_mut 阶段 3 登记不变 |
