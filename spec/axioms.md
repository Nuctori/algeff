...
注 1（数学审计 M1）：pdr.md §四 A3 原文含「且不存在 Write/Own 模式重叠」——为冗余合取：`Δ(a)∩Δ(b)=∅` 已蕴含无共享键，模式重叠无从谈起，故陈述框省略该子句（Cov(Δ) 已涵盖其语义）。
...
- A7 的分层状态（数学审计 M4 修正）：**模型层已交付**（`tla/scheduler.tla` 4 不变式 + Progress）；**原语层已交付**（`ResourceArbiter`：原子占坑 + 失败回滚 + 有限重试，arbiter.rs 8 项测试）；**执行器层已接入**（A5 批 7：`op_mutex_lock` 经 arbiter `try_claim` + 有限重试 + WouldBlock 上抛）；**运行时调度层**：静态冲突 → 顺序化 + 并行分支资源不相交 + 单执行器锁——无死锁由「冲突→顺序 + 分支不相交 + 单执行器互斥」成立（比原「动态占坑」论证更强且更平凡；动态占坑原论证的运行时载体为 arbiter 接入后的执行器层）。
