...
**新增风险（A6 交付质量问题，非空窗）**：
- ~~`a4_random_read_write_sequence` 把 `Write→Own` 误判为「重复消费应拒绝」……该测试当前确定性失败（3/3 复现）~~ → **批 2 误报，已更正（§5.1）**：批 2 记录时 worktree 基于修复前（`caf6611`）的 `axioms.rs`；CTO 修复 `91396c2` 已对齐冻结语义（Write→Own 合法，`resource.rs::linearity_write_then_own_legal`），main 22 测试全绿。原风险条目撤销。
- 次要：core 的 `reactor` 字段 warning（never read）在 A2 合并 interpret 后自然消除；在此之前 clippy 保持 0 error，G2 判据不受影响。

## 5. 批 3 状态刷新与 G2 审计门禁最终检查项（2026-08 更新）

### 5.1 issue-1 更正（批 2 误报）

批 2 记录的「main 红」系**误报**：批 2 复核时 worktree 基于修复前（`caf6611`）的
`axioms.rs`，`a4_random_read_write_sequence` 当时将 `Write→Own` 判为「重复消费应拒绝」
而确定性失败。CTO 修复 `91396c2`（A6 线性 proptest 对齐 A3 合并后的 Own 终结语义）后，
测试已与冻结语义一致（Write→Own 合法，同 `resource.rs::linearity_write_then_own_legal`），
...
对照 `contracts.md` §4 G2（A1 审计报告闭环 + `cargo clippy --workspace` 无 error）
与 `contracts-audit.md` §5 审计发现闭环要求，G2 放行前逐项核销：

**A. 审计发现闭环（contracts-audit.md §5 建议逐条）**

- [x] D13 补录 contracts.md §3 决策表（`a16380f` 已执行）
- [x] DataOp 变体计数 39→36（`a16380f` 已执行）
- [ ] D12 偏差裁决（推荐修订 pdr.md §2.3 词法规范化，待 CTO 裁决；非阻塞）
- [ ] D5 语义记录（contracts.md D5 行补「内存管道」定义，可选非阻塞，待 CTO）
- [x] A7/P5 载体闭环：`tla/scheduler.tla` 已交付且 TLC 通过（4 不变式 + 1 时序属性）；
      interpret 动态占坑/回滚/重试已随 A2 合并 main（`003acd7`）

**B. 契约门禁（contracts.md §4 G2 原文）**

- [x] A1 审计报告闭环：`spec/contracts-audit.md` 已交付，偏差已记录（D5/D12 语义权威化）
- [x] `cargo clippy --workspace` 无 error（本 worktree 复核：0 error，core 1 warning：reactor）
- [x] `cargo test --workspace` 全绿（main 22 测试绿；本 worktree 复核通过）

**C. 执行级闭环（§3 G2 判据，逐项须「已验证」）**

- [ ] A1 结合律（interpret Sequential trace 等价）——载体已合并 main（`003acd7`），待执行
- [ ] A2 执行级（Pure 跳过/单位元、UndoStack 不压栈）——同上
- [ ] A3 动态调度（Fork 并行/串行降级；D14 阶段 1 顺序执行语义）——同上
- [ ] A4 执行级（interpret 接入 check_linear；Write→Own 合法序列）——同上
- [ ] A5（Choose 隔离 + Fork COW `make_mut`）——`make_mut` 载体待 A2/A3 确认
- [ ] A6 端到端（algeff-std Full 策略文件往返）——阻塞于 A5 TokioExecutor 未合并
- [ ] A7 动态（占坑/回滚/重试压力测试）——interpret 已合并 main，待压力测试落地
- [ ] P1–P5 对应执行级项（P1/P2/P4/P5 随上述载体；P3 随 A5）
- [x] TLA 模型检测通过（已满足：`NoCircularWait` 等 4 不变式 + `Progress` 时序属性）

**D. 门禁放行定义**

G2 = A 全部核销（或 CTO 对 D12/D5 裁决记录在案）∧ B 全绿 ∧ C 全部「已验证」。
当前 C 组中 A2/A3/A4/A7 的执行级验证随解释器合并 main 后已具备运行条件，
A5/A6/P3 仍待 A5 TokioExecutor 合并——**G2 放行需 A5 合并后复核 C 组**。
...
| A2/A3/A4/A6 静态+属性 | `crates/algeff-core/tests/axioms.rs` 22 项 | **22 通过（全绿）**（`91396c2` 修复后；批 2 误报已更正，§5.1） |
