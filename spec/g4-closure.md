...
> **R6 附录（2026-08-16，历史记录之上的更新注记）**：本文为 G4 时点快照，数字系当时实测（157 测试、append 24.3%）。R6 审计-修复闭环后：测试累计 **352/49 块全绿**；append 复测 **39.1%**（af27ce9 D-039 写后 flush 契约后诚实值，24.3% 为修复前旧数，详见 `perf/baseline-r6-2026-08-16.txt`）；echo 99.9% 无回归；跨平台错误语义（JD-1 kind-first）与线性失败回滚（RFC-12）已修复。本文历史行不逐一改写，以本节注记 + `docs/src/derivation.md` §9 为准。
...
| # | 残余项 | 现状 | 裁决建议 |
| --- | --- | --- | --- |
| **R-1（C4 延续）** | D16 arbiter ↔ `op_mutex_lock` 接入 | **已落地并核销（A5 批 7 `254eaf3` + 批 8 `f897669` + 批 9 `4c993f3`，D-034）**：`try_claim` + 8×1ms 有限重试 + WouldBlock；RAII claim guard（取消路径释放）；arbiter 改 std Mutex（Drop 内 async panic 消除，blocker-1 解除）；强制记录已入 resource-notes §2；语义变更已入契约 D16 | ✅ **核销**（D-034；A5 批 9 合并后复验通过） |
...
**观察项（不阻塞）**：R-3（make_mut 阶段 3）、R-6（executor 锁串行化，接受，阶段 3+ 工程缓解）；R-1 已核销（D-034）、R-2/R-4 已交付、R-5 已修复（D19）。
