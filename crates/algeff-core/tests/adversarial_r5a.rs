...
/// `run_sub_impl` 每层 +1，超 96 即返回 `Other(105)`。故 depth=95 的叶子在
/// depth 95（<96）正常执行；depth=96/97 的叶子/中途节点在 depth 96 触发守卫。
...
// ══════════════════════════════════════════════════════════════════════
// 攻击面 2a：守卫边界精确压力 —— 深度 95/96/97（95 成功、96/97 Err(Other(105))）
// ══════════════════════════════════════════════════════════════════════

/// 边界下沿 94/95 成功 + 上沿 96/97 触发守卫（Err(Other(105))）：
/// 守卫不误伤合法嵌套、且在栈溢出之前触发。进程不 abort（测试能跑完即证明）。
#[test]
fn guard_boundary_95_ok_96_97_err() {
    let mut rt = Runtime::new(Box::new(NoopExecutor));

    // 下沿：94/95 均正常执行（95 是阈值下沿 —— 叶子恰在 depth 95）。
    assert_eq!(
        rt.run_blocking(nested_seq(94)).unwrap(),
        Value::U64(300),
        "深度 94 应在守卫阈值（96）之下正常执行"
    );
    assert_eq!(
        rt.run_blocking(nested_seq(95)).unwrap(),
        Value::U64(300),
        "深度 95（阈值下沿）应正常执行，守卫不误伤合法嵌套"
    );

    // 上沿：96/97 均在深度 96 触发守卫 → Err(Other(105))（ENOBUFS 语义近似）。
    assert_eq!(
        rt.run_blocking(nested_seq(96)).unwrap_err(),
        SysError::Other(105),
        "深度 96（阈值上沿）应返回深度守卫错误，收到非 Other(105)"
    );
    assert_eq!(
        rt.run_blocking(nested_seq(97)).unwrap_err(),
        SysError::Other(105),
        "深度 97 同样应返回深度守卫错误"
    );
    ...
    assert_eq!(
        rt.run_blocking(nested_seq(95)).unwrap(),
        Value::U64(300),
        "守卫触发后同 Runtime 再执行下沿深度 95 应成功（守卫不粘滞）"
    );
}
...
#[test]
fn guard_depth96_wrapped_catch_handler_receives_105() {
    let mut rt = Runtime::new(Box::new(NoopExecutor));
    ...
    assert_eq!(
        ...
        "Catch 应捕获深度守卫错误（深度 96）并执行 handler"
    );
