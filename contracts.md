...
| # | 决策 | 理由 |
| --- | --- | --- |
| D1 | `Fd = u64` 全局唯一句柄 | pdr.md §2.3「Fd 由运行时分配全局唯一句柄，避免重用冲突」；i32 只是示意 |
...
| D16 | `ResourceArbiter`：动态资源仲裁原语（原子占坑+失败回滚，A7 工程载体），静态 can_parallel 管 Fork 级、动态 arbiter 管 MutexLock 级 | 审计补录；资源仲裁分层无循环等待 |
