...
5. **验证**：435 个测试函数（约 422 个二进制测试 + 13 条 doc-test）+ 51 个测试二进制，叠加
   `tla/scheduler.tla` 模型检测；
6. **审计**：8 轮「对抗审计（203 个 E2E 测试）× 形式逻辑审计」——P1/P2/P3/P5 终判
...
| 对抗 E2E | `tests/adversarial_r{1..7}.rs` | 203 个测试（R1=21、R2=23、R3=28、R4=33、R5=24、R6=42、R7=15、R7AB=4、R8=13） |
| 模型检测 | `tla/scheduler.tla` | TLC 通过 `TypeOK`/`ExclusiveHold`/`ExactHold`/`NoCircularWait` 4 不变式 + `Progress` 时序属性 |

总量：`cargo test --workspace` **435 个测试函数**（约 422 个 `#[test]`/`#[tokio::test]` +
13 条 doc-test 断言；51 个测试二进制 + 3 个 doc-test 运行）。特性测试（coeffects/virtual-clock）
由 feature 门控，CI 三平台补跑。

### 6.2 审计协议（5 轮串行 + R6 并行扩展）
...
| append（10 任务 × 32KB 顺序追加） | 6.0947 ms | 1.4814 ms | **24.3%**（R6 复测：2.3618 ms → **39.1%**，af27ce9 D-039 flush 契约后诚实值，见 `perf/baseline-r6-2026-08-16.txt`） | 29.4% |
