...
| R7 | ✅ 完成（adversarial_r7.rs 15 测试，迭代 2，E2E 累计 182）：RFC-05/06/07/08/09 修复面 + R-6 fork_snapshot + DX 层六面攻击；发现：dx mutex 不可重入（已修）、TcpShutdown infer_usage（已修，后裁决空集） | ✅ 完成（formal R7，迭代 2）：**P4 升级「有效（附范围声明）」**——RFC-05 句柄活性反例闭环 + 取消传播（undo 回滚/线性快照/锁释放）证据齐备；P1 承诺域 ≤64 与代码一致；A4 双回滚（取消×失败）无冲突；do_! 展开 ≡ 手写 CPS 链等价性成立 | dx mutex 空声明修复；SendSignal 可重复；TcpShutdown Own→空集（40a62a6）；P4 升级证据链 | **迭代 2 收官**：P1/P2/P3/P5 有效附声明维持、**P4 升级有效（附范围声明）**——A1-A7 × P1-P5 全部有效/部分中仅剩 P4 范围例外（R7-A/B 宽限耗尽）与 P3 物理层（make_mut 阶段 3） |
...
| A4 资源线性（use/move 拆分，D-0xx） | Write 不限次数（use，运行时维护独立逆操作）；Own 恰好终结一次（move） | spec/proofs.md（修订注：P1-P5 不依赖 Write 消费） | undo_semantics_contract.rs:sequential_multi_write_same_fd_allowed_use_semantics（二写允许 + LIFO 撤销）+ mutex_reentry_still_blocked_by_arbiter_after_a4_use_semantics（mutex 防重入归 A7）+ axioms.rs:a4_write_use_semantics_repeatable + a4_random_read_write_sequence（Own 终结后拒绝）+ adversarial_r1.rs:lin_fork_conflict_double_write_then_parent_blocked（Fork 后父级 Write 允许）+ lin_stale_fd_write_after_replace_fails（RFC-05：Replace 后旧 fd Write → NotFound） | 轮 R1/R3/R7 ✅ + D-0xx use/move 拆分 | ✅ 有效（use/move 拆分后：Write 消费维度取消，Own 终结维度保留；RFC-05 句柄回收闭合不变） |
